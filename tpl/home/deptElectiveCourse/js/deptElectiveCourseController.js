;(function(window, undefined) {
	...
	window.index_deptElectiveCourseController = function($scope, $state, $timeout, $cookies, $http, $uibModal, $compile, $rootScope, $window, choose_deptElectiveCourseService, alertService, app) {
	    ...
	    //获取当前学年学期
		choose_deptElectiveCourseService.getCurrentSemester(function (error, message, data){
			if (error) {
	    ...
	    $scope.queryParams = function queryParams(params) {
            ...
            return angular.extend(pageParam, $scope.deptElectiveCourse);
	    };
	    
		$scope.deptElectiveCourseTable = {
//			url: app.api.address + '/choose/publicElectiveCourse',
			...
			onLoadSuccess: function() {
				$compile(angular.element('#deptElectiveCourseTable').contents())($scope);
				if($scope.refreshTable){
					$timeout(function(){
	        			angular.element('#deptElectiveCourseTable').bootstrapTable('selectPage', 1);
		            },800);
...
//				$("body").fadeOut();
//				$timeout(function(){
//					$state.go("home.common.deptClassChoose");
//	            },500);
//				$("body").fadeIn(800);
//	        },
...
//          	}
			...
			columns: [
				{field:'number',title :'序号',align:"center",valign:"middle",width:"5%",
                    formatter : function(value, row, index) {  
                        var page = angular.element('#deptElectiveCourseTable').bootstrapTable("getPage");  
		...
		$scope.chooseButton = function(){
			...
			choose_deptElectiveCourseService.add(param, function (error, message) {
				$rootScope.showLoading = false; // 关闭加载提示
				if (error) {
					alertService(message);
					return;
				}
				angular.element('#deptElectiveCourseTable').bootstrapTable('refresh');
	...
	index_deptElectiveCourseController.$inject = ['$scope', '$state', '$timeout', '$cookies', '$http', '$uibModal', '$compile', '$rootScope', '$window', 'choose_deptElectiveCourseService', 'alertService', 'app'];
