;(function(window, undefined) {
	...
	window.index_physicalEducationClassController = function($scope, $state, $timeout, $cookies, $http, $uibModal, $compile, $rootScope, $window, choose_physicalEducationClassService, alertService, app) {
	    ...
	    //获取当前学年学期
		choose_physicalEducationClassService.getCurrentSemester(function (error, message, data){
			if (error) {
	    ...
	    $scope.queryParams = function queryParams(params) {
            ...
            return angular.extend(pageParam, $scope.physicalEducationClass);
	    };
	    
		$scope.physicalEducationClassTable = {
//			url: app.api.address + '/choose/physicalEducationClass',
        	url: 'data_test/index/tableview_physicalEducationClass.json',
			method: 'get',
			cache: false,
			...
			onLoadSuccess: function() {
				$compile(angular.element('#physicalEducationClassTable').contents())($scope);
				if($scope.refreshTable){
					$timeout(function(){
	        			angular.element('#physicalEducationClassTable').bootstrapTable('selectPage', 1);
		            },800);
...
//				$("body").fadeOut();
//				$timeout(function(){
//					$state.go("home.common.physicalClassChoose");
//	            },500);
//				$("body").fadeIn(800);
//	        },
			columns: [
				{field:'number',title :'序号',align:"center",valign:"middle",width:"5%",
                    formatter : function(value, row, index) {  
                        var page = angular.element('#physicalEducationClassTable').bootstrapTable("getPage");  
				...
				{field: "optionalClassNum",title: "可选班级数",align: "center",valign: "middle",width:"10%"},
				{field: "courseType",title: "课程类别",align: "center",valign: "middle",width:"15%"},
				{field: "courseProperty",title: "课程性质",align: "center",valign: "middle",width:"10%"},
				{field: "openDeptName",title: "开课单位",align: "center",valign: "middle",width:"10%"},
		...
		$scope.chooseButton = function(){
			...
			choose_physicalEducationClassService.add(param, function (error, message) {
				$rootScope.showLoading = false; // 关闭加载提示
				if (error) {
					alertService(message);
					return;
				}
				angular.element('#physicalEducationClassTable').bootstrapTable('refresh');
	...
	index_physicalEducationClassController.$inject = ['$scope', '$state', '$timeout', '$cookies', '$http', '$uibModal', '$compile', '$rootScope', '$window', 'choose_physicalEducationClassService', 'alertService', 'app'];
